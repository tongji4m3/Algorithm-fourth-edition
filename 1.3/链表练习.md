...
**1.3.26**
```java
public Node remove(Node x, String key)
{
	//传入的Node引用最后可能与x不同，所以返回的x才是修改过的
	//（因为x.next将导致两个引用指向链表的不同部分）
	if (x == null)
		return x;
	if (x.next == null)
	{
	...
	while (x.item == key) // 使得头不为key
	{
		x = x.next;
	}
	Node current;
	for (current = x; current != null && current.next != null;)
	{
